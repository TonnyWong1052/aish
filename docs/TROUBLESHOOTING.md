...
#### Issue: PowerShell execution policy prevents running install script
...
1. **Check and update execution policy:**
```powershell
# Check current policy
Get-ExecutionPolicy

# Allow script execution for current user (recommended)
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser

# Or bypass policy for single execution
PowerShell -ExecutionPolicy Bypass -File .\scripts\install.ps1
```

2. **Alternative: Manual installation without scripts:**
```powershell
# Build binary directly
go build -o aish.exe .\cmd\aish

# Create bin directory and copy
New-Item -ItemType Directory -Path "$env:USERPROFILE\bin" -Force
Copy-Item aish.exe "$env:USERPROFILE\bin\"

# Add to PATH
$currentPath = [Environment]::GetEnvironmentVariable('Path', [EnvironmentVariableTarget]::User)
if ($currentPath -notlike "*$env:USERPROFILE\bin*") {
    [Environment]::SetEnvironmentVariable('Path', "$currentPath;$env:USERPROFILE\bin", [EnvironmentVariableTarget]::User)
}

# Initialize AISH
& "$env:USERPROFILE\bin\aish.exe" init
```

#### Issue: Windows Defender or antivirus blocking aish.exe

**Symptoms:**
- Binary gets quarantined after build
- Execution blocked by SmartScreen
- Hook installation fails silently

**Solutions:**

1. **Add exclusions:**
```powershell
# Add exclusion for AISH directory (requires administrator)
Add-MpPreference -ExclusionPath "$env:USERPROFILE\bin"
Add-MpPreference -ExclusionPath "$env:USERPROFILE\.config\aish"

# Or add specific file exclusion
Add-MpPreference -ExclusionPath "$env:USERPROFILE\bin\aish.exe"
```

2. **Bypass SmartScreen (one-time):**
- When SmartScreen blocks: Click "More info" → "Run anyway"

#### Issue: Hook not working in PowerShell

**Symptoms:**
- Commands fail but AISH doesn't trigger
...
```powershell
# AISH requires PowerShell 5.1 or later
$PSVersionTable.PSVersion

# If older, download latest PowerShell from:
# https://github.com/PowerShell/PowerShell/releases
```

3. **Manually reinstall hook:**
```powershell
...
```powershell
# Some modules may interfere with prompt hooks
# Check loaded modules
Get-Module

# If PSReadLine is missing (required for better capture):
Install-Module -Name PSReadLine -Scope CurrentUser -Force
```

#### Issue: PATH not updating after installation

**Symptoms:**
- `aish` command not found after installation
- Need to use full path to run aish

**Solutions:**

1. **Restart PowerShell terminal** - PATH changes require new session

2. **Manually update PATH for current session:**
```powershell
$env:Path += ";$env:USERPROFILE\bin"
```

3. **Verify PATH is set permanently:**
```powershell
# Check user PATH
...
#### Issue: Output capture not working on Windows

**Symptoms:**
- AISH doesn't see command output
...
Test-Path "$env:USERPROFILE\.config\aish"

# Check if files are being written
Get-ChildItem "$env:USERPROFILE\.config\aish" -Filter "last_*"
Get-Content "$env:USERPROFILE\.config\aish\last_stderr"
```

2. **Enable verbose capture (for debugging):**
```powershell
...
3. **Install PSReadLine for better capture:**
```powershell
# PSReadLine improves command history and capture
Install-Module -Name PSReadLine -Scope CurrentUser -Force
