...
func TestNewPresenter(t *testing.T) {
	...
	if presenter.spinner != nil {
		t.Error("NewPresenter should initialize with nil spinner")
	}
}
...
func TestPresenterFields(t *testing.T) {
	...
	// Test that presenter has the expected structure
	if presenter.spinner != nil {
		t.Error("New presenter should have nil spinner")
	}
	
	// Test that we can access the spinner field
	presenter.ShowLoading("test")
	if presenter.spinner == nil {
		t.Error("After ShowLoading, spinner should not be nil")
	}
	
	presenter.StopLoading(true)
	if presenter.spinner != nil {
		t.Error("After StopLoading, spinner should be nil")
	}
}