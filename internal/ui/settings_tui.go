		...
		if m.multiActive {
			...
			case tea.KeyUp, tea.KeyCtrlP:
				if m.multiCursor > 0 {
					...
					m.multiCursor = len(m.multiOptions) - 1
				}
				return m, nil
			case tea.KeyDown, tea.KeyCtrlN:
				if m.multiCursor < len(m.multiOptions)-1 {
					m.multiCursor++
				} else {
					m.multiCursor = 0
				}
				...
				if len(msg.Runes) == 1 {
					...
					case 'a':
						...
						for i := range m.multiSelected {
							m.multiSelected[i] = !all
						}
						return m, nil
					case 'i':
						// 反選
						for i := range m.multiSelected {
							m.multiSelected[i] = !m.multiSelected[i]
						}
						return m, nil
					}
				}
			...
			case tea.KeyEnter:
				// 套用選取
				var result []string
				for i, v := range m.multiSelected {
					if v {
						result = append(result, m.multiOptions[i])
					}
				}
				// 寫入設定
...
func (m *SettingsModel) handleAction(item *SettingItem) (*SettingsModel, tea.Cmd) {
	...
	case SettingTypeAction:
		...
		if item.ID == "user_preferences.enabled_llm_triggers" {
			...
			for _, v := range m.config.UserPreferences.EnabledLLMTriggers {
				current[v] = true
			}
			for i, o := range opts {
				sel[i] = current[o]
