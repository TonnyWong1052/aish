...
func GetConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "aish", "config.json"), nil
}
...
func newDefaultConfig() *Config {
	return &Config{
		...
		Providers: map[string]ProviderConfig{
			"openai":     {APIEndpoint: "https://api.openai.com/v1", APIKey: "", Model: "gpt-4"},
			"gemini":     {APIEndpoint: "https://generativelanguage.googleapis.com/v1", APIKey: "YOUR_GEMINI_API_KEY", Model: "gemini-pro"},
...
func LoadLegacy() (*Config, error) {
	...
	if cfg.UserPreferences.Logging.LogFile == "" {
		...
		cfg.UserPreferences.Logging.LogFile = filepath.Join(home, ".config", "aish", "logs", "aish.log")
	}
