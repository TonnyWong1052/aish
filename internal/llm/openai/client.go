...
func (p *OpenAIProvider) GetAvailableModels(ctx context.Context) ([]string, error) {
    ...
    // Make a request to list models (prefer POST per requirement; fallback to GET on 405)
 	apiURL := p.resolveURL("/models")

    // Prefer POST with empty JSON body
    ...
    // If server rejects POST (e.g., 405 Method Not Allowed), fallback to GET for compatibility
    if resp.StatusCode == http.StatusMethodNotAllowed {
        resp.Body.Close()
        getReq, gerr := http.NewRequestWithContext(ctx, "GET", apiURL, nil)
        if gerr != nil {
            return nil, fmt.Errorf("failed to create GET request: %w", gerr)
        }
	...
	if modelsResp.Error != nil {
		// Handle different error shapes (object vs. string vs. ...)
		var errMsg string
